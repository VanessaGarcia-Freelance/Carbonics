...
switch($action) {
	...
	case 'duplicate' :
		...
		$blocks = isset($_REQUEST['aq_blocks']) ? $_REQUEST['aq_blocks'] : '';
		$this->duplicate_template($selected_template_id, $blocks, $template_name);
	...
	<div id="page-builder-frame">
		...
		<div id="page-builder-column" class="metabox-holder <?php echo $disabled ?>">
			<div id="page-builder-archive" class="postbox">
				<h3 class="hndle"><span><?php _e('Available Blocks', 'framework') ?></span><span id="removing-block"><?php _e('Deleting', 'framework') ?></span><span class="min"><i class="fa fa-angle-down"></i></span><span class="max"><i class="fa fa-angle-up"></i></span></h3>
				<div class="inside">
				...
				<div class="aqpb-tabs-nav">
					...
					<div class="aqpb-tabs-wrapper">
						<div class="aqpb-tabs">
							...
							<span class="aqpb-tab aqpb-tab-add aqpb-tab-active"><abbr title="Add Template"><i class="fa fa-plus-square"></i></abbr></span>
							<?php } else { ?>
							...
							?>">
								<abbr title="Add Template"><i class="fa fa-plus-square"></i></abbr>
							</a>
							<?php } ?>
								...
								<div id="submitpost" class="submitbox">
									<div class="major-publishing-actions cf">
										...
										<label class="open-label" for="template-name">
											...
											<h5>[template id="<?php echo $selected_template_id ?>"]</h5>
										</label>
										...
										<div class="publishing-action">
											<span>Save</span>
											<button type="submit" name="save_template" id="save_template_header" class="button button-primary "><i class='fa fa-save'></i></button>
										...
										<div class="duplicate-action">
											<span>Duplicate</span>
											<?php 
											echo '<a class="submitduplocate duplication template-duplicate" href="' . esc_url(add_query_arg(
												array(
													'page' => $this->args['page_slug'], 
													'action' => 'duplicate',
													'template' => $selected_template_id,
										...
										<div class="add-action">
											<span>New</span>
											<?php if(!empty( $selected_template_id )) {echo '<a class="new" href="'.admin_url().'themes.php?page=aq-page-builder&amp;action=edit&amp;template=0"><abbr title="Add Template"><i class="fa fa-file-text-o"></i></abbr></a>';} ?>
										</div>
						...
						<div id="aqpb-body">
							...
							<ul class="blocks cf" id="blocks-to-edit">
								...
								if($selected_template_id === 0) {
									...
									echo __('<b><u>How To Use</u></b><br/><br/> Click on the \'New\' button to create a empty template. Enter a title and click \'Save\'.
									<br/><br/>
									Drag and drop blocks from the \'Available Blocks\' container to the page builder area. You can reorder these block to your preference. No resizing of blocks required.
									<br/><br/>
									When you have finished building your custom page template, make sure that you\'ve clicked the \'Save\' button.', 'framework');
									echo '</p>';
