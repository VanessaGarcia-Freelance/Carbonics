	...
	class AQ_Page_Builder {
		...
		**/
		function duplicate_template($template_id, $blocks, $title) {
			...
			//wp security layer
			check_admin_referer( 'duplicate-template', '_wpnonce' );
			...
			if(!get_page_by_title( $title, 'OBJECT', 'template' )) {
				...
				//create the template
				$dup_template_id = wp_insert_post($template);
			...
			$post_dup = get_post($dup_template_id, ARRAY_A);
			
			$key_value = get_post_meta( $template_id, $key, true );
			
			foreach ($key_value as $key => $value) {
				foreach ($value as $key2 => $value2) {
					$val = unserialize($value[$key2]);
					add_post_meta($dup_template_id, $key, $val);
				} 
			}
		}
